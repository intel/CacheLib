...
CacheAllocator<CacheTrait>::replaceChainedItemLockedForMoving(Item& oldItem,
  ...
  // to drop the refcount the parent holds on oldItem by manually calling
  // decRef.  To do that safely we need to have a proper outstanding handle.
  //WriteHandle oldItemHdl{};
  //auto incRes = incRef(oldItem, false);
  //XDCHECK(incRes == RefcountWithFlags::incResult::incOk);
  //oldItemHdl = WriteHandle{&oldItem,*this};
  //XDCHECK_EQ(2u, oldItemHdl->getRefCount()); 
  ...
  while ((config_.evictionSearchTries == 0 ||
    ...
    if (!evictedToNext) {
      ...
      if (candidate->isChainedItem()) {
        ...
        // if parent changed then the below assert will be false
      ...
      if (chainedItem) {
          ...
          evictedToNext.reset(); //destroy new chained item hdl
          ...
          if (parentHandle) {
            ...
            XDCHECK(false);
            const auto res =
                releaseBackToAllocator(*toRecycleParent, RemoveContext::kNormal);
            XDCHECK(res == ReleaseRes::kReleased);
          }
      } else {
...
bool CacheAllocator<CacheTrait>::moveForSlabRelease(
  ...
  //TODO djb need to check here that the item is chained
  //and should we mark mogin
...
CacheAllocator<CacheTrait>::allocateNewItemForOldItem(const Item& oldItem) {
  ...
  if (oldItem.isChainedItem()) {
    ...
    auto& expectedParent = oldChainedItem.getParentItem(compressor_);
    const auto parentKey = expectedParent.getKey();

    // Grab lock to prevent anyone else from modifying the chain
    auto l = chainedItemLocks_.tryLockExclusive(parentKey);
    
    auto *parentItem = &oldItem.asChainedItem().getParentItem(compressor_);
    XDCHECK(parentItem->isMoving());
    // safe to acquire handle for a moving Item
    auto incRes = incRef(*parentItem, false);
    XDCHECK(incRes == RefcountWithFlags::incResult::incOk);
    auto parentHandle = WriteHandle{parentItem,*this};
    XDCHECK_EQ(1u, parentHandle->getRefCount()); 
...
bool CacheAllocator<CacheTrait>::tryMovingForSlabRelease(
  ...
  if (getNumTiers() == 1) {
    ...
    bool ret = chainedItem
        ? moveChainedItemWithSync(oldItem.asChainedItem(), newItemHdl, parentHandle)
        : moveRegularItemWithSync(oldItem, newItemHdl);
    if (!ret) {
      //we failed to move - newItemHdl was released back to allocator
      //by the handle but oldItem is not accessible
      //and no longer valid - it will be cleaned up in the caller
      XDCHECK(!oldItem.isAccessible());
      if (chainedItem) {
        XDCHECK(!oldItem.isMoving()); //we didn't even get to mark moving
        //we can fail if parent changed or failed to get lock to move
        //we should try to evict the entire chain like in findEviction
        auto& parent = oldItem.asChainedItem().getParentItem(compressor_);
        XDCHECK(parent.isMoving());
        ret = parent.markForEvictionWhenMoving();
        XDCHECK(ret);
        unlinkItemForEviction(parent);
        wakeUpWaiters(parent, {});
        const auto res =
            releaseBackToAllocator(parent, RemoveContext::kNormal, false);
        XDCHECK(ReleaseRes::kReleased == res);
        return ret;
      }
    }
...
void CacheAllocator<CacheTrait>::evictForSlabRelease(
  ...
  while (true) {
    ...
    if (item.isChainedItem()) {
      ...
      if (!l || expectedParent.getKey() != parentKey || 
              !item.isInMMContainer() || item.isOnlyMoving() ||
      ...
      if (evicted->markForEviction()) {
        ...
        const auto res =
          releaseBackToAllocator(*evicted, RemoveContext::kNormal, false);
        XDCHECK(ReleaseRes::kReleased == res);
      } else {
...
bool CacheAllocator<CacheTrait>::markMovingForSlabRelease(
  ...
  const auto fn = [&markedMoving, &itemFreed, numTiers, this](void* memory) {
