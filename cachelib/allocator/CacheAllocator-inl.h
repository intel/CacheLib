...
bool CacheAllocator<CacheTrait>::moveRegularItemWithSync(
  ...
  // ANSWER from djb: yes but a chained item could end up with unaccessible parent
  // in slab relase path, so we need to clean up
...
bool CacheAllocator<CacheTrait>::moveChainedItemWithSync(ChainedItem& oldItem,
  ...
  XDCHECK(parentItem.isMoving());
  XDCHECK(l);
  replaceChainedItemLockedForMoving(oldItem, newItemHdl, parentItem);
  return true;
}
...
bool CacheAllocator<CacheTrait>::tryMovingForSlabRelease(
   ...
   bool ret = oldItem.isChainedItem()
              ? moveChainedItemWithSync(oldItem.asChainedItem(), newItemHdl,
                                        oldItem.asChainedItem().getParentItem(compressor_))
  ...
  const auto fn = [this, tid, &syncItem, &markedMoving, &itemFreed](void* memory) {
    ...
    mmContainer.withContainerLock([this, &syncItem, &item, &markedMoving]() {
      ...
      if (chainedItem_ &&
          ...
          markedMoving = false;
          //TODO: could we still potentially race on isMMContainer()? or no
          //      because we are holding lock - which means no other thread
          //      may remove from mmContainer right now
          return;
      }
