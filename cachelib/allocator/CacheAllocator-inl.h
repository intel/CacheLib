...
template <typename CacheTrait>
size_t CacheAllocator<CacheTrait>::backgroundWorkerId(PoolId pid,
                                                      ClassId cid,
                                                      size_t numWorkers) {
  XDCHECK(numWorkers);
...
CacheAllocator<CacheTrait>::allocateInternal(PoolId pid,
  ...
  if (backgroundEvictor_.size() && !fromBgThread &&
    ...
    backgroundEvictor_[backgroundWorkerId(pid, cid, backgroundEvictor_.size())]
        ->wakeUp();
  }
...
template <typename CacheTrait>
auto CacheAllocator<CacheTrait>::getAssignedMemoryToBgWorker(
    size_t evictorId, size_t numWorkers) {
  ...
  for (const auto pid : pools) {
    ...
    for (const auto cid : mpStats.classIds) {
      if (backgroundWorkerId(pid, cid, numWorkers) == evictorId) {
        asssignedMemory.emplace_back(pid, cid);
      }
    }
  }
  ...
  for (size_t i = 0; i < threads; i++) {
    ...
    if (result) {
      backgroundEvictor_[i]->setAssignedMemory(
          getAssignedMemoryToBgWorker(i, backgroundEvictor_.size()));
    }
  }
  return result;
}
  ...
  for (size_t i = 0; i < threads; i++) {
    ...
    if (result) {
      backgroundPromoter_[i]->setAssignedMemory(
          getAssignedMemoryToBgWorker(i, backgroundPromoter_.size()));
    }
  }
  return result;
}
