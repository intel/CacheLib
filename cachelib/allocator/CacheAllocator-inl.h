...
CacheAllocator<CacheTrait>::findEviction(TierId tid, PoolId pid, ClassId cid) {
  ...
  while ((config_.evictionSearchTries == 0 ||
    ...
    if (!evictedToNext) {
      ...
      maybeWakeUpWaiters(*candidate, std::move(evictedToNext));
    }
...
template <typename CacheTrait>
typename RefcountWithFlags::Value CacheAllocator<CacheTrait>::unmarkMovingAndMaybeWakeUpWaiters(Item &item, WriteHandle handle)
{
  auto ret = item.unmarkMoving();
  maybeWakeUpWaiters(item, std::move(handle));
  return ret;
}
...
bool CacheAllocator<CacheTrait>::moveForSlabRelease(
  ...
  for (unsigned int itemMovingAttempts = 0;
    ...
    if (oldItem.isOnlyMoving()) {
      auto ret = unmarkMovingAndMaybeWakeUpWaiters(oldItem, {});
      XDCHECK(ret == 0);
      const auto res =
  ...
  auto ref = unmarkMovingAndMaybeWakeUpWaiters(oldItem, std::move(newItemHdl));
  XDCHECK(ref == 0);
...
template <typename CacheTrait>
void CacheAllocator<CacheTrait>::maybeWakeUpWaiters(Item& item, WriteHandle handle)
{
...
void CacheAllocator<CacheTrait>::evictForSlabRelease(
  ...
  while (true) {
    ...
    if (shutDownInProgress_) {
      auto ref = unmarkMovingAndMaybeWakeUpWaiters(item, {});
